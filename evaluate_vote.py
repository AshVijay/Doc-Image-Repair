...
def evaluate_vote(img_path,output_path,model_path,lineseg_path,charseg_path):
    ...
    RoI_img = Image.open(para.data_result_path+'/RoI_results/RoI.png')
    ...
    for char in chars:
        ...
        #elif voters[min_idx][2]<0.2 and voters[min_idx][1]>0.5:
        #    original_patch=healthy_patches0[np.argwhere(healthy_labels==voters[min_idx][0])][0][0]
        #    repair_patch=Shift(original_patch,voters[min_idx][3],voters[min_idx][4])
        else:
            voters_patches=[]
            for voter in voters:
        ...
        output[line_count*40*multi+40*2:line_count*40*multi+40*3,char[0]:char[1]+1]=np.stack((repair_patch,)*3, axis=-1)
        output[line_count*40*multi+40*3:line_count*40*multi+40*4,char[0]:char[1]+1]=repaired_patch
